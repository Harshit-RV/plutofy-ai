 ...
 */
export const createApiKey = async ({ name, access = "ALL", token } : { name: string, access?: string, token: string}) : Promise<ApiKeyDoc> => {
  try {
 ...
 */
export const getApiKeys = async (token : string) : Promise<ApiKeyDoc[]> => {
  try {
 ...
 */
export const deleteApiKey = async ({ apiKeyId, token } : { apiKeyId: string, token: string}) => {
  try {
    await axios.post(

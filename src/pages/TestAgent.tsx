...
const AgentCreate = ({ isTestMode = false } : { isTestMode: boolean }) => {
  ...
  const [ testingMode, setTestingMode ] = useState<TestingMode>(TestingMode.DEVELOPER_MODE);
  ...
  return (
    !agentLoading || agent != undefined ? (
      <div className='flex flex-col font-mono min-h-screen bg-gray-100'>
          <div className={`flex justify-between items-center bg-white border ${pagePadding} pt-5 w-full py-5`}>
          ...
          <div className={`flex flex-col gap-5 flex-grow ${pagePadding} pt-4 w-full`}>
            <Collapsible
              open={isConfigOpen}
            ...
            <Tabs defaultValue={isTestMode ? "test" : "usage"}>
              <TabsList className="bg-gray-200 border-gray-300 border">
                <TabsTrigger className="px-3" value="usage">Usage</TabsTrigger>
                <TabsTrigger className="px-4" value="test">Test</TabsTrigger>
                <TabsTrigger className="px-3" value="settings">Settings</TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="test" className="flex flex-col gap-7 flex-grow pt-4 w-full">
                <div className="flex items-center space-x-2">
                  <Switch id="airplane-mode" checked={testingMode == TestingMode.DEVELOPER_MODE} onCheckedChange={(val) => setTestingMode(val ? TestingMode.DEVELOPER_MODE : TestingMode.NORMAL_MODE)}/>

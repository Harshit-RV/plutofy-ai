...
import { AgentDoc, AgentProps, CompletionResponse } from '../types/agent';
...
class AgentService {
   ...
   */
  static createAgent = async ( {agentProps, token} :{agentProps: Omit<AgentProps, 'userId'>, token: string}): Promise<AgentDoc> => {
    try {
   ...
   */
  static getAgentsByUserId = async (token: string): Promise<AgentDoc[]> => {
    try {
   ...
   */
  static getAgentByDocId = async ( { agentDocId, token }:{ agentDocId: string, token: string }): Promise<AgentDoc> => {
    try {
      const response = await axios.get(
   ...
   * @param token - The authentication token for the request
   */
  static deleteAgent = async ( { agentId, token } : {agentId: string, token: string}) => {
    try {
      await axios.post(
   ...
   */
  static getAllAgents = async (token: string): Promise<AgentDoc[]> => {
    try {
      const response = await axios.get(
        `${API_URL}/agent/all`, 
        {
   ...
   */
  static updateAgent = async ({ agentId, updateFields, token } : { agentId: string, updateFields: Partial<AgentProps>, token: string }): Promise<AgentDoc> => {
    try {
      const response = await axios.post(
  ...
  static getCompletion = async <T>(body: T, token: string): Promise<CompletionResponse> => {
    try {
      const response = await axios.post(
